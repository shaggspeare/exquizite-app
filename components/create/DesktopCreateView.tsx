...
export function DesktopCreateView() {
  ...
  const addWordPair = () => {
    ...
    setWordPairs([...wordPairs, { id: Date.now().toString(), word: '', translation: '' }]);
  };

  const updateWordPair = (id: string, field: 'word' | 'translation', value: string) => {
    setWordPairs(pairs =>
  ...
  const handleAIWordsSelected = (aiWords: WordPair[]) => {
    ...
    if (wordsToAdd.length < aiWords.length) {
      showAlert('Limit Reached', `Only ${wordsToAdd.length} words were added to stay within the ${MAX_WORDS_PER_SET} word limit.`);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && editingSetId) {
        await updateSet(editingSetId, finalSetName, validPairs, preferences.targetLanguage, preferences.nativeLanguage);
        showAlert('Success', 'Set updated successfully!', [
          { text: 'OK', onPress: () => router.push(`/sets/${editingSetId}`) },
        ]);
      } else {
        const newSet = await createSet(finalSetName, validPairs, preferences.targetLanguage, preferences.nativeLanguage);
        if (newSet) {
          clearForm();
  ...
  const filledPairs = wordPairs.filter(p => p.word.trim() && p.translation.trim());
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <DesktopContainer>
          <View style={styles.headerContent}>
            ...
            >
              ...
              <Text style={styles.saveButtonText}>{saving ? 'Saving...' : 'Save Set'}</Text>
            </TouchableOpacity>
          </View>
        </DesktopContainer>
      </View>
        ...
        <DesktopContainer>
          <View style={styles.twoColumnLayout}>
            ...
            <View style={styles.leftColumn}>
              <Input label="Set Name" value={setName} onChangeText={setSetName} placeholder="e.g., Spanish Vocabulary" />
              ...
              <View style={styles.wordsSection}>
                ...
                {wordPairs.map(pair => (
                  <WordPairInput
                    ...
                    onChangeTranslation={text => updateWordPair(pair.id, 'translation', text)}
                    onDelete={() => deleteWordPair(pair.id)}
                    canDelete={wordPairs.length > 1}
                  />
                ))}

                <TouchableOpacity style={styles.addButton} onPress={addWordPair} disabled={wordPairs.length >= MAX_WORDS_PER_SET}>
                  <Ionicons name="add-circle-outline" size={24} color={wordPairs.length >= MAX_WORDS_PER_SET ? colors.textSecondary : colors.primary} />
                  <Text style={[styles.addButtonText, { color: wordPairs.length >= MAX_WORDS_PER_SET ? colors.textSecondary : colors.primary }]}>
                    Add Word
                  </Text>
                </TouchableOpacity>

                <TouchableOpacity
                  style={[styles.aiButton, { backgroundColor: `${colors.ai}10`, borderColor: colors.ai }]}
                  onPress={() => setShowAIModal(true)}
                  disabled={wordPairs.length >= MAX_WORDS_PER_SET}
                >
                  <Ionicons name="sparkles" size={24} color={wordPairs.length >= MAX_WORDS_PER_SET ? colors.textSecondary : colors.ai} />
                  <Text style={[styles.aiButtonText, { color: wordPairs.length >= MAX_WORDS_PER_SET ? colors.textSecondary : colors.ai }]}>
                    AI Suggestions
                  </Text>
                </TouchableOpacity>
              </View>
              ...
              <Card style={styles.previewCard}>
                ...
                <View style={styles.previewContent}>
                  <View style={styles.previewHeader}>
                    <Text style={[styles.previewSetName, { color: colors.text }]}>
                      {setName.trim() || 'Untitled Set'}
                    </Text>
                    <View style={styles.previewStats}>
                      <Ionicons name="book-outline" size={16} color={colors.textSecondary} />
                      <Text style={[styles.previewStatsText, { color: colors.textSecondary }]}>
                        {filledPairs.length} {filledPairs.length === 1 ? 'word' : 'words'}
                      </Text>
                    </View>
                  </View>
                  ...
                  {filledPairs.length > 0 ? (
                    <View style={styles.previewList}>
                      {filledPairs.slice(0, 5).map((pair, index) => (
                        <View key={pair.id} style={[styles.previewItem, { borderBottomColor: colors.border }]}>
                          <Text style={[styles.previewWord, { color: colors.text }]}>{pair.word}</Text>
                          <Text style={[styles.previewTranslation, { color: colors.textSecondary }]}>{pair.translation}</Text>
                        </View>
                      ))}
                      {filledPairs.length > 5 && (
                        <Text style={[styles.previewMore, { color: colors.textSecondary }]}>
                          +{filledPairs.length - 5} more {filledPairs.length - 5 === 1 ? 'word' : 'words'}
                        </Text>
                      )}
                    </View>
                  ) : (
      ...
      <AISuggestionModal visible={showAIModal} onClose={() => setShowAIModal(false)} onSelectWords={handleAIWordsSelected} existingPairs={wordPairs} />
    </View>
  );
}
